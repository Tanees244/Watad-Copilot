...
// Embed products directly into the system instruction text
const systemInstruction = `
  ...
  - Image: https://staging.watad.vip/_next/image?url=https%3A%2F%2Fwatad-staging-bucket.s3.eu-west-3.amazonaws.com%2FWindow%2520Air%2520Condition.jpg&w=750&q=75
  - Form Fields:
    1. Unit of measurement (text, optional)
    2. Quantity (number, required)
    3. Do you want to split your order? (select, false/true, optional)
    4. When would you like to receive your order? (text, required)
    5. Upload file (file, optional)
    6. Any other information? (textarea, optional)

  Product 2:
  - Title: Male Adapter Hexagonal
  - Category: Plumbing
  - Subcategory: Pipes & Fittings
  - Image: https://staging.watad.vip/_next/image?url=https%3A%2F%2Fwatad-staging-bucket.s3.eu-west-3.amazonaws.com%2FMale%2520Adapter%2520Hexagonal.jpg&w=750&q=75
  - Form Fields:
    1. Unit of measurement (text, optional)
    2. Quantity (number, required)
    3. Do you want to split your order? (select, false/true, optional)
...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const productMatch = userMessage.toLowerCase().includes("window air condition") 
        ? { title: "Window Air Condition", category: "HVAC & Refrigeration", subcategory: "Split Air Conditioners & Window Air Conditioners" } 
        : userMessage.toLowerCase().includes("male adapter hexagonal")
        ? { title: "Male Adapter Hexagonal", category: "Plumbing", subcategory: "Pipes & Fittings" }
        : null;
